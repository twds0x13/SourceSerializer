...
手写 block 可以调用 `SerializerRegistry` 的全部 13 种内置类型的 public static Scan/Emit 方法，以及 `GeneratedSerializers` 中 SG 生成的 `Scan_Xxx`/`Emit_Xxx` 方法。
...
- [SerializerRegistry API](../api/serializer-registry): 13 种内置类型的 Scan/Emit 方法
