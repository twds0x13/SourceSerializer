...
flowchart TD
    ...
    Q --> R["WhitespaceStripper 预处理<br/>Deserialize/TryScan 自动构造"]
```
...
直接调用 `block.Scan` 时，输入 blank 不会被自动剔除——如果需要空白符容忍，应使用便捷方法 `Deserialize<T>()` 或 `TryScan<T>()`，它们在调用 `Scan` 前自动构造 `WhitespaceStripper` 进行预处理，使得调用方无需关心输入中的空白符。
